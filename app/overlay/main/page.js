...
function MainOverlayContent() {
  ...
  useEffect(() => {
    ...
    const poll = setInterval(() => {
      if (!booyahDeclared.current) fetchTeamsOnly()
    }, 3000)
    return () => {
  ...
  function setupRealtime() {
    ...
    channelRef.current = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'overlay_settings' },
        () => fetchSettings())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'themes' },
        () => { if (liveMatchIdRef.current) fetchTheme(liveMatchIdRef.current) })
      .subscribe()
  }

  // This is the KEY fix — always fetch fresh complete data, never merge
  async function fetchTeamsOnly() {
    ...
    // Always fetch completely fresh data — no merging
    ...
    // Replace state completely — no merging old data
    setTeams(data)
  ...
  async function fetchSettings() {
    if (!matchId) return
    const { data } = await supabase
      .from('overlay_settings').select('*').eq('match_id', matchId).single()
    if (data) {
      setSettings(data)
  ...
  async function fetchTheme(mId) {
    const { data } = await supabase
      .from('themes').select('*').eq('match_id', mId).single()
  ...
  async function fetchPoints() {
    ...
    const { data: mp } = await supabase
      .from('match_points').select('*').eq('match_id', matchId)
    setMatchPoints(mp || [])
  }
  ...
  function getRowBg(rank, isElim) {
    if (rank === 1) return 'linear-gradient(90deg,rgba(30,22,0,0.97),rgba(10,8,4,0.93))'
    if (rank === 2) return 'linear-gradient(90deg,rgba(20,20,22,0.97),rgba(8,8,12,0.93))'
    if (rank === 3) return 'linear-gradient(90deg,rgba(22,14,4,0.97),rgba(8,8,12,0.93))'
    if (isElim) return 'linear-gradient(90deg,rgba(20,5,5,0.95),rgba(8,8,12,0.90))'
    return 'linear-gradient(90deg,rgba(12,9,4,0.96),rgba(8,8,12,0.92))'
  }
      ...
      <>
        ...
        <main style={{ minHeight:'100vh', backgroundColor:by.bg||'#000', display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden', position:'relative' }}>
          <div style={{ position:'absolute', inset:0, overflow:'hidden', pointerEvents:'none' }}>
            {[...Array(6)].map((_,i) => (
              <div key={i} style={{ position:'absolute', top:'-50%', left:`${i*20-10}%`, width:'8%', height:'200%', background:`${by.accentColor||'#10b981'}08`, transform:'rotate(25deg)', animation:`diagonalMove ${3+i*0.5}s linear infinite` }}/>
            ))}
          </div>
              ...
              {aliveTeams.map((team, index) => {
                ...
                return (
                  <div key={team.id} style={{
                    ...
                    <div style={{ width:28, height:28, borderRadius:4, background:`${rc}22`, border:`1px solid ${rc}44`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, marginLeft:6 }}>
                      <span style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:10, fontWeight:900, color:rc }}>
                        {team.name?.slice(0,2).toUpperCase()}
                      </span>
                    </div>

                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:15, fontWeight:900, letterSpacing:'1.5px', textTransform:'uppercase', color:'#fff', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis', lineHeight:1, marginBottom:2 }}>
                        {team.name}
                      </div>
                      <div style={{ display:'flex', alignItems:'center', gap:4 }}>
                        <span style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:9, fontWeight:700, color:'rgba(255,255,255,0.35)', letterSpacing:'1.5px', textTransform:'uppercase' }}>WIN</span>
                        <span style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:13, fontWeight:900, color:rc, letterSpacing:'0.5px' }}>{winPct}%</span>
                      </div>
                    </div>
