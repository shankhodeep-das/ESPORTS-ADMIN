    ...
    <>
      <style>{`
        ...
        @keyframes cardSlideIn {
          0%   { transform: translateX(-30px) scaleX(0.9); opacity: 0; }
          70%  { transform: translateX(3px)   scaleX(1.01); opacity: 1; }
          100% { transform: translateX(0)     scaleX(1);   opacity: 1; }
        }
        @keyframes swipeSwap {
          0%   { transform: translateY(0);     opacity: 1; }
          35%  { transform: translateY(-24px); opacity: 0; }
          65%  { transform: translateY(24px);  opacity: 0; }
          100% { transform: translateY(0);     opacity: 1; }
        }
        @keyframes liveBlink {
          0%,100% { opacity:1; }
      ...
      >
        ...
        {/* ══════════════════ FINAL 4 — BGMI BROADCAST STYLE TOP BAR ══════════════════ */}
        {showFinal4 && overlayState === 'final4' && (
          <div style={{
            ...
            alignItems: 'center',
            animation: final4Visible ? 'f4SlideDown 0.7s cubic-bezier(0.34,1.2,0.64,1) forwards' : 'none',
          }}>

            {/* Cards row — full width centered */}
            <div style={{
              display: 'flex',
              ...
              {aliveTeams.map((team, index) => {
                ...
                // accent color per rank — same as reference image
                const accentColor =
                  index === 0 ? '#e63946' :   // red for 1st
                  index === 1 ? '#e63946' :   // red
                  index === 2 ? '#e63946' :   // red
                                '#e63946'     // red (all red like BGMI broadcast)
                  ...
                  <div
                    ...
                    style={{
                      ...
                      animation: final4Visible
                        ? isSwap
                          ? 'swipeSwap 0.7s cubic-bezier(0.4,0,0.2,1)'
                          : `cardSlideIn 0.55s cubic-bezier(0.34,1.2,0.64,1) ${index * 0.09}s both`
                        : 'none',
                    }}
                  >
                    {/* Main card */}
                        ...
                        <div style={{ display: 'flex', gap: '3px' }}>
                          {Array.from({ length: totalPl }, (_, i) => (
                            <div key={i} style={{
                              ...
                              boxShadow: i < alivePl ? `0 0 4px ${accentColor}60` : 'none',
                            }}/>
                          ))}
                        </div>

                        {/* Row 4: Win % label — shows alive count like reference */}
                        <div style={{
                          display: 'flex', alignItems: 'center',
                          justifyContent: 'space-between',
                          marginTop: '5px',
                          ...
                          <span style={{
                            ...
                            color: alivePl > 0 ? '#ffffff' : 'rgba(255,255,255,0.3)',
                            letterSpacing: '1px',
                          }}>
                            {alivePl}/{totalPl}
                          </span>
            ...
            <div style={{
              ...
              <div style={{ width: 5, height: 5, borderRadius: '50%', background: '#e63946', animation: 'liveBlink 1s infinite' }}/>
              <span style={{
                fontFamily: "'Barlow Condensed',sans-serif",
